<?php
 ...
 try {
   ...
   $fileNameToMove = sprintf('./uploads/%s-%s.%s',$fileName['filename'],date('Y-m-d-h-i-s'),$ext);
   $fileNameToSave = $fileName['filename'] .'-'. date('Y-m-d-h-i-s') . '.' . $ext;

   if (!move_uploaded_file(
       $_FILES['certImg']['tmp_name'],$fileNameToMove
   )) {
